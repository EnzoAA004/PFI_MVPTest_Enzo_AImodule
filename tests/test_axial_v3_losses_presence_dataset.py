...
def test_25d_dataset_edges_and_cross_patient_protection() -> None:
    records = [
        SliceRecord25D("p1_0", "m1_0", "train", "p1", "s1", "0", 0),
        SliceRecord25D("p1_1", "m1_1", "train", "p1", "s1", "1", 1),
        SliceRecord25D("p2_0", "m2_0", "val", "p2", "s1", "0", 0),
    ]
...
def test_25d_blocks_unknown_or_duplicated_order() -> None:
    missing = [SliceRecord25D("a", "m", "train", "p", "s", "0", None)]
    with pytest.raises(ValueError, match="missing reliable slice order"):
        require_reliable_slice_order(missing)
    duplicated = [
        SliceRecord25D("a", "m", "train", "p", "s", "0", 0),
        SliceRecord25D("b", "m", "train", "p", "s", "1", 0),
    ]
